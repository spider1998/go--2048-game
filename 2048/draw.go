...
func drawCell(left, top int, width, height int, val string) {
	...
	for l := left; l < left+width; l++ {
		for t := top; t < top+height; t++ {
			termbox.SetCell(l, t, ' ', termbox.ColorWhite, termbox.Attribute(10+int(math.Log2(valF))))
		}
	}
	//填充单元格中的数字
	drawText(val, width, TextMiddle, left, top+height/2, termbox.ColorRed, termbox.Attribute(10+int(math.Log2(valF))))
}
